<template>
  <v-layout row justify-center>
                      ...
                      <v-container grid-list-md>
                        ...
                        <v-layout wrap>
                          <v-flex xs12 sm12 md12>
                            <v-text-field
                              v-validate="
                                'required|PerfilNombreUnicoCrear|max:80'
                              "
                        ...
                        <v-flex md12>
                          <v-card>
                            <ag-grid-vue
                              ...
                              @selection-changed="onSelectedChangeModulos"
    ...
    <app-confirmacion
      ...
      :titulo="$t('mensajes.crud.crear.titulo', { entidad: 'Perfil' })"
      :mensaje="$t('mensajes.crud.crear.mensaje', { entidad: 'el Perfil' })"
...
export default class PerfilesModalNuevo extends ComponenteBaseStepper {
  ...
  async obtenerRolesAsignados(usuarioId: number) {
    try {
      this.responserolesUsuario = await this.axios.http.get(
  ...
  private asociarModulosValidador() {
    const loadingKey = 'asociarModulosValidador';
  ...
  doesExternalFilterPassModulo(node: RowNode): boolean {
    if (
      this.modulosAccionRespaldo.length > 0 &&
      this.buscarModulosNuevo.length > 1
    ) {
      let busqueda = this.buscarModulosNuevo.toLowerCase();
      return (
      ...
      this.$validator.validateAll().then(res => {
        if (res) {
          ...
          let model = {
            ...
            PerfilRoles: this.editedItem.perfilRoles,
            PerfilUsuarios: this.editedItem.perfilUsuarios,
          };
          ...
          this.axios.http
            ...
            .then(res => {
              ...
              this.grabaAsignacionModulosAccion(res.data.id);
            })
            .catch(error => {
              this.loadingService.hideLoading();
              this.btnBloquear = false;
  ...
  grabaAsignacionModulosAccion(idPerfil: number) {
    ...
    if (this.accionesSeleccionadasArbol.length > 0) {
      ...
      this.modulosAccionAsignados.forEach((modulo: any) => {
        ...
        //Módulos sin Acciones
        /*else {
          let existeModuloCheck = this.accionesSeleccionadasArbol.find(
            x => x === modulo.id
          );
      ...
      this.axios.http
        .post(
          `${this.funcionalidadAccionController}/${idPerfil}/grabaFuncionalidadAccionesNuevo`,
          model
        )
        .then(res => {
          this.loadingService.hideLoading();
        ...
        .catch(err => {
          ...
          if (err.response !== undefined) {
            ...
            this.$snotify.error(
              this.$t('mensajes.mensajesError.crear', { entidad: 'el Perfil' })
            );
          }
        });
    } else {
  ...
  async obtenerFuncionalidadAcciones() {
    ...
    try {
      ...
      this.modulosAccionRespaldo = JSON.parse(response.data);

      //[COPIAR PEGAR] Track 3.0 - Grillas
      this.modulosAccionRespaldo.forEach((func: any) => {
        func.estadoTxt =
  ...
  habilitaCheckConsultar() {
    ...
    if (this.accionesGuardadas.length != 0) {
      this.accionesSeleccionadasArbol = this.accionesGuardadas;
    }
  }
