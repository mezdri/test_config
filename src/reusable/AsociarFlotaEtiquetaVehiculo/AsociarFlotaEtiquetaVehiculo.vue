...
export default class AsociarFlotaEtiquetaVehiculo extends Vue {
  ...
  async handleSelectedFlotasChange(flotas: Flota[]) {
    //console.log(flotas);
      ...
      await this.axiosPostgres.http
        ...
        .then((response: any) => {
          if (response.data.CODIGO_RESPUESTA == '1') {
            this.vehiculos.seleccionados = response.data.vehiculos;
          } else {
  ...
  onReportTypeChanged(oldValue: any, newValue: any) {
    if (oldValue == 'estado_transmision' || newValue == 'estado_transmision') {
      this.cargarAsociaciones();
    }
  }
