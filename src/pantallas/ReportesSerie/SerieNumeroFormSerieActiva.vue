        ...
        >
          <v-container grid-list-md style="padding: 30px 40px 50px 40px;">
            <FormFieldsContainer titulo="Detalle de Preparación">
              ...
              <v-flex xs4 sm4 md4>
                <v-text-field
                  :label="formFields.fecha_instalacion.label"
                  v-model="formFields.fecha_instalacion.value"
                  :disabled="modoVer"
                />
              </v-flex>
              <v-flex xs4 sm4 md4>
...
export default class SerieNumeroFormSerieActiva extends Vue {
  ...
  async getSerieActiva_by_id(
    ...
    return fetchReportesSerie(
      ...
      'ReportesSerie/serieactiva'
    )
      .then((data: any) => {
        if (data.data.CODIGO_RESPUESTA == '1') {
          if (data.data.data[0].epc.length !== 0) {
            self.formFields.epc.value = data.data.data[0].epc;
          } else {
          ...
          if (data.data.data[0].estado.length !== 0) {
            self.formFields.fecha_instalacion.value = data.data.data[0].estado;
          } else {
