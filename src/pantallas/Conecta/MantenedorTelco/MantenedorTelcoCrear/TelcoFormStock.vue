...
export default class TelcoFormStock extends ComponenteBase {
  ...
  readonly funcionalidad: Funcionalidades =
    Funcionalidades.MantenedorTelcoCrearStock;
  ...
  async getReportes(telcoid_crear: any) {
    let self = this;
    this.loadingService.showLoading();
    return fetchReportesSerie(
      { telcoid: self.telcoid_crear },
  ...
  async handleGridReady(event: GridReadyEvent) {
    this.ordesList = await this.getReportes(this.telcoid_crear);
  ...
  accionEditarStockCrear: PermisoAccion = {
    accionCodigo: 102,
    visible: false,
  };

  accionEliminarStockCrear: PermisoAccion = {
    accionCodigo: 103,
    visible: false,
  };
  ...
  accionesTotales: PermisoAccion[] = [
    ...
    this.accionEditarStockCrear,
    this.accionEliminarStockCrear,
  ];
  constructor() {
    super();
    actionsEvent.onEditarStockCrear(this.handleVerEditarStock);
    actionsEvent.onEliminarStockCrear(this.handleElminarStock);
  }
  handleVerEditarStock(params: ICellRendererParams) {
    (this.formFields.simid.value = params.data.tiposimid),
      ...
      (this.telcoid_crear = this.telcoid_crear);
  ...
  handleElminarStock(params: ICellRendererParams) {
    (this.tipoSimIdEliminar = params.data.tiposimid),
      (this.telcoid_crear = params.data.telcoid);
