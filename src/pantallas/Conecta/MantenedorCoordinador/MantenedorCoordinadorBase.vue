<template>
  <div class="reportes-coordinadores">
    <div class="flex sm12 m12">
      <FormFieldsContainer>
        <v-flex sm12 md12 layout wrap justify-space-between mb-4>
          ...
          <v-flex class=" xs12 sm6 md6 layout-flex-coor">
      ...
      <app-confirmacion
        ...
        :mensaje="`¿Estás Seguro de que desas eliminar a ${data.coorname}?`"
        v-on:modalConfirmationEmit="handleDeleteCoor($event)"
        :textoBtnSi="$t('mensajes.crud.eliminar.btnAceptar')"
        :textoBtnNo="$t('mensajes.crud.eliminar.btnCancelar')"
      >
      </app-confirmacion>

      <ModalViewCoordinador
        v-if="showModal"
...
import { fetchMantenedorCoordinador } from './fetchMantenedorCoordinador';
import { PermisoAccion } from '@/views/base/VariablesPermisos';
import { actionsEvent } from '@/setup';
import Confirmation from '@/components/Confirmation.vue';

import ModalViewCoordinador from './ModalViwCoordinador.vue';
...
export default class MantenedorCoordinadorBase extends ComponenteBase {
  ...
  async recargarGrilla() {
    this.ordesList = await this.getCoordinadores();
  ...
  readonly gridOptions: GridOptions = {
    columnDefs: [
      ...
      {
        headerName: 'Apellido Coordinador',
        field: 'coorapellido',
        cellStyle: { textAlign: 'left' },
        resizable: true,
        width: 300,
      },
      {
        ...
        field: 'coorfecha_alta',
        cellStyle: { textAlign: 'left' },
        resizable: true,
        width: 300,
        cellRenderer: (param: any) =>
          param.data.coorfecha_alta.substring(0, 19),
      },
      {
        headerName: 'Estado',
        field: 'coorbaja',
        ...
        cellRenderer: (params: any) => {
          if (params.data.coorbaja === 'Vigente') {
            return ` <i class="material-icons" style="width: 15px; height: 15px;margin-top: 7px; border-radius: 50%; background: rgb(42, 255, 78)"></i>
                <a style="color: #4f4f4f; position: absolute; margin-top: 0%; margin-left: 3%">${params.data.coorbaja}</a>
              `;
          } else {
  ...
  showViewModal(params: ICellRendererParams, addaction: boolean = false) {
    if (addaction) {
      this.data = {};
      this.anulado = false;
    } else if (params.data.coorbaja != 'Vigente') {
  ...
  handleDeleteCoor(event: any) {
    this.dialogEliminar = false;
    if (event) {
      this.deletecoordinador(this.data);
    }
  }
  ...
  async getCoordinadores() {
    ...
    return fetchMantenedorCoordinador(
      ...
      'api_coordinadores/get_coordinadores'
    )
      .then((data: any) => {
  ...
  async deletecoordinador(params: any) {
    const { coorid, coorname, coorapellido } = params;
    let self = this;
    this.loadingService.showLoading();
    return fetchMantenedorCoordinador(
      {
        coorid: Number(coorid),
        coorname: coorname,
        coorapellido: coorapellido,
        usuarioid_modifica: getUserId(),
        usuariorut_modifica: localStorage.getItem('clienteDefecto'),
      },
      'api_coordinadores/delete_coordinador'
    )
      .then((data: any) => {
        if (data.data.CODIGO_RESPUESTA == '1') {
...
.reportes-coordinadores .estilo-grilla,
#gridComponent {
