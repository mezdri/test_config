<template>
  <div id="mainDiv">
    ...
    <v-flex
      sm3
      md3
      style="display: flex; align-items: flex-end; justify-content: flex-end; max-width: 100%; right: 40px;"
      v-if="showButtonInformes"
    >
      <v-btn style="height: 48px;" color="#ffe800" @click="handleModal">
        <v-icon dark left style="font-size: 160%; margin-right: 7% !important;"
...
export default class InformesGenerados extends Vue {
  ...
  showButtonInformes: boolean = false;

  mounted() {
    this.showButtonInformes =
      this.$route.path == '/Torre-Control/informes_generados';
  }
  constructor() {
    super();
    ...
    this.columnDefs = [
      ...
      },
      {
        ...
        cellRenderer: (params: any) => {
          return params.data.is_programmed == 'true' &&
  ...
  getHtml(params: any): string {
    ...
    if (
      params.data.is_programmed == 'true' &&
      params.data.estado == 'PENDIENTE'
    ) {
      css =
        '<i class="material-icons" id="5545" title="Pendiente" style="width: 24px; height: 24px; margin-top: 7%; background: rgb(39, 72, 142); border-radius: 50%;"></i>';
      return (
        css +
  ...
  async onGridReady(params: GridReadyEvent) {
    ...
    this.serverSide = {
      getRows(params: any): void {
        ...
        } else if (self.$route.path == '/Torre-Control/informes_generados') {
          body.aplication = 'TC_INFORME_GENERADO';
        }
  ...
  handleModal() {
    this.loadingService.showLoading();
    this.$router.push('/Torre-Control/generar_informe');
  }
}
</script>
