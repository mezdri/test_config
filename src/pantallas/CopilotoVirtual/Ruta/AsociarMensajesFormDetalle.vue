...
export default class AsociarMensajesFormDetalle extends Vue {
  @Prop({ default: (): [] => [] })
  ...
  @Prop({ required: false }) lstEventoMensajes: any[];
  @Prop({ required: false }) rutaSeleccionada: Ruta;
  /* state */
  ...
  @Watch('lstEventoMensajes', { deep: true })
  handleEventosMensajes() {
    this.asignaMensajes();
  }
  ...
  asignaMensajes() {
    ...
    this.axiosCopiloto.http
      ...
      .then((response: any) => {
        ...
        this.lstEventoMensajes.forEach(i => {
          if (i.e1 == 'Sí') {
            let ix = 1;
        ...
        this.axiosCopiloto.http
          ...
          .then((response: any) => {
            ...
            this.grillaEventoMensajes.forEach(g => {
              let d = datos.find((d: any) => d.rutaEventoId == g.idRutaEvento);
              if (d != undefined) {
                this.$set(g, 'rutaEventoMensajeId', d.rutaEventoMensajeId);
                this.$set(g, 'idMensaje', d.id);
                this.$set(g, 'mensaje', d.titulo);
              } else {
            ...
            this.gridApiEventosMensajes.setRowData(this.grillaEventoMensajes);
          })
          .finally(() => this.loadingService.hideLoading());
      });
  }
