...
export default class VehiculosGrid extends ComponenteBase {
  ...
  columnDefs: Array<ColDef> = [
    {
      ...
      lockPosition: true,
      hide: undefined,
      cellRenderer: this.createButtonIcon,
    },
    {
      headerName: 'Estado GPS',
      field: 'ultimaActividad.transmitiendo',
      width: 30,
      minWidth: 30,
      resizable: true, //true,
      sortable: true, //true,
      ...
      lockPinned: true,
      hide: undefined,
      cellRenderer: this.createIconRD,
    },
    {
      headerName: 'Estado Ignición',
      field: 'ultimaActividad.ignicion',
      width: 30,
      minWidth: 30,
      resizable: true, //true,
      sortable: true, //true,
      ...
      lockPinned: true,
      hide: undefined,
      cellRenderer: this.createCorteIgnicionCell,
    },
    {
      headerName: 'Corta Corriente',
      colId: 'cortaCteID',
      field: 'estadoCortaCorriente',
      width: 30,
      minWidth: 30,
      sortable: true, //true,
      cellStyle: { padding: 0 },
      ...
      hide: undefined,
      lockPinned: true,
      lockVisible: this.accionCortaCorriente.visible,
      resizable: true,
      cellRenderer: this.createCortaCorrienteCell,
    },
    {
      headerName: 'Unidad',
      field: 'unidad',
    ...
    },
    {
      ...
      field: 'ultimaActividad.ubicacion',
      width: 100,
      minWidth: 100,
    ...
    },
    {
      ...
      field: 'ultimaActividad.proveedor',
      width: 100,
      minWidth: 100,
      cellStyle: { textAlign: 'left' },
    },
  ...
  gridOptions: GridOptions = {
    ...
    onDragStopped: this.guardarEstadoGrilla,
    onColumnVisible: this.guardarEstadoGrilla,
    onColumnPinned: this.guardarEstadoGrilla,
    deltaRowDataMode: true,
  ...
  onGridReady(params: GridReadyEvent) {
    ...
    this.cargarVehiculos().then(
      this.agGridUtil.recuperarEstadoGrilla.bind(this.agGridUtil)
    );
  }
  ...
  onCellClicked(event: CellClickedEvent) {
    ...
    if (!vehiculo.marcaGps || vehiculo.marcaGps == 'Enfora') {
  ...
  createButtonIcon({ data: vehiculo }: { data: Vehiculo }) {
    ...
    eSpan.addEventListener('click', () => {
      localStorage.setItem('idVehiculoRdia', vehiculo.id.toString());
      localStorage.setItem('patenteVehiculoRdia', vehiculo.patente.toString());
      localStorage.setItem(
        'fechaActividadModal',
        vehiculo.fechaUltimaActividad.toString()
      );

      rDiaEvents.$emit('abrirModalRdia', true);
    });
    return eSpan;
  }
  ...
  createCortaCorrienteCell({ data: vehiculo }: { data: VehiculoHomeGrid }) {
    ...
    if (!vehiculo.marcaGps || vehiculo.marcaGps == 'Enfora') {
      eSpan.style.cssText =
        'cursor:default;color:#cccccc;font-size: 18px;disabled:true';
    } else if (vehiculo.estadoCortaCorriente == '6') {
      eSpan.style.cssText = 'cursor:pointer;color:red;font-size: 18px;';
    } else {
      eSpan.style.cssText = 'cursor:pointer;color:green;font-size: 18px;';
    }
