...
import {
  ...
  FetchCanceler,
  fetchActividades,
} from '@/pantallas/Inicio/ActividadesData/fetchActividades';
...
export default class ActividadesGridSugal extends Vue {
  ...
  async cargarActividades() {
    ...
    if (!this.vehiculoSeleccionado) {
      ...
      actividadesGrilla = actividades.filter(
        (x: Actividad) =>
          x.fechaActividad <= this.vehiculoSeleccionado.fechaUltimaActividad
      );
    }
  ...
  async actualizarActividades(vehiculo: Vehiculo) {
    ...
    let actividades: Actividad[] = await fetchActividades(
      this.vehiculoId,
      this.setCancelFetchActividades
    );
  ...
  async mounted() {
    ...
    repetirDiaEventBus.$on(repetirDiaEvents.abrir, () => {
      ...
      console.log(this.estadoColumnas);
    });
