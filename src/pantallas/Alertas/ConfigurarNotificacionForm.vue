<template>
  <div align="center">
    <v-flex md10>
      <v-card class="rounded-card">
        <v-form ref="form" lazy-validation class="col-md-12">
          ...
          <v-container style="padding: 10px 10px 10px 10px;">
            <FormFieldsContainer titulo="Medio de Notificación">
              <slot name="prependMedioNotificacion" />
              <v-flex xs12 sm4 md4>
                <v-checkbox
                  :label="formFields.notificaGps.label"
                  v-model="formFields.notificaGps.value"
                  :data-vv-name="formFields.notificaGps.id"
                  :disabled="modoVer"
                ></v-checkbox>
              </v-flex>
              <v-flex xs12 sm4 md4>
                <v-checkbox
                  :label="formFields.notificaBrowser.label"
                  v-model="formFields.notificaBrowser.value"
                  :data-vv-name="formFields.notificaBrowser.id"
                  :disabled="modoVer"
                ></v-checkbox>
              </v-flex>
              <v-flex xs12 sm4 md4>
                <v-checkbox
                  :label="formFields.notificaEmail.label"
                  v-model="formFields.notificaEmail.value"
                  :data-vv-name="formFields.notificaEmail.id"
                  :disabled="modoVer"
                ></v-checkbox>
              </v-flex>
            </FormFieldsContainer>
          ...
          <v-container grid-list-md text-xs-center>
            <StepTitle>Calendario</StepTitle>
            <v-divider></v-divider>
            <v-layout row wrap>
              <v-flex xs12 sm6 md2 offset-xs10 align-end>
                <AgregarButton
                  v-if="!modoVer"
                  @click="handleCrearCalendario()"
                />
              </v-flex>
            </v-layout>
            <v-layout row wrap>
              <v-flex md12>
                <v-card>
                  ...
                  <MantenedorGridVerDetalle
                    v-else
                    :grid-options="gridOptionsCalendario"
                    :row-data="lstCalendarioSeleccionado"
                  />
                </v-card>
              </v-flex>
            </v-layout>
    ...
    <CrearCalendarioModal
      ...
      @nuevoCalendario="handleNuevoCalendario"
    />
  </div>
</template>
...
export default class ConfigurarNotificacionForm extends Vue {
  ...
  readonly gridOptionsCalendario: GridOptions = {
    ...
    rowSelection: 'single',
    onGridReady: this.handleGridCalendarioReady,
    onSelectionChanged: this.handleSeleccionCalendario,
  };
  ...
  setListaDistribucionSeleccionadas() {
    ...
    if (this.alertaSeleccionada) {
      if (
        this.alertaSeleccionada.notificacionListaDistribucion &&
        this.alertaSeleccionada.notificacionListaDistribucion.length > 0
      ) {
        this.alertaSeleccionada.notificacionListaDistribucion.forEach(l => {
  ...
  setContactosSeleccionados() {
    ...
    if (this.alertaSeleccionada) {
      if (
        this.alertaSeleccionada.notificacionContacto &&
        this.alertaSeleccionada.notificacionContacto.length > 0
      ) {
        this.alertaSeleccionada.notificacionContacto.forEach(l => {
  ...
  setUsuariosSeleccionados() {
    ...
    if (this.alertaSeleccionada) {
      if (
        this.alertaSeleccionada.notificacionUsuario &&
        this.alertaSeleccionada.notificacionUsuario.length > 0
      ) {
        this.alertaSeleccionada.notificacionUsuario.forEach(l => {
  ...
  handleFormChange(fields: FormFieldDefMap) {
    emitFormChangeEvent(this.$emit.bind(this), {
      entity: {
        notificaGps: fields.notificaGps.value,
        notificaBrowser: fields.notificaBrowser.value,
  ...
  handleAlertaSeleccionadaChange() {
    if (this.alertaSeleccionada) {
      this.formFields.notificaGps.value = this.alertaSeleccionada.notificaGps;
      this.formFields.notificaBrowser.value = this.alertaSeleccionada.notificaBrowser;
      this.formFields.notificaEmail.value = this.alertaSeleccionada.notificaEmail;
    }
  }
  ...
  fetchLstCalendario() {
    ...
    fetchCalendarios()
      .then(res => {
        this.lstCalendario = res;
        this.gridApiCalendario.setRowData(this.lstCalendario);
        this.setCalendariosSeleccionados();
      })
      .catch(error => {
        if (error.response !== undefined) {
          if (error.response.status !== 401) {
            this.$snotify.error(
              'Ocurrió un error al obtener lista de calendario'
            );
          }
        } else {
          this.$snotify.error(
  ...
  async handleGridCalendarioReady(event: GridReadyEvent) {
    this.gridApiCalendario = event.api;
    this.fetchLstCalendario();
  }

  handleSeleccionCalendario() {
    var seleccion: number[] = [];
    if (this.gridApiCalendario.getSelectedRows().length > 0) {
      this.gridApiCalendario.getSelectedRows().forEach(c => {
        seleccion.push(c.id);
      });
    }

    this.$emit('calendarioSelect', seleccion);
  }
  ...
  handleNuevoCalendario(calendario: CalendarioNuevoForm) {
    this.loadingService.showLoading();

    if (!this.lstCalendario) {
      this.lstCalendario = [];
    }

    calendario.fechaInicioTxt = calendario.fechaInicio
      ? this.formatFecha(calendario.fechaInicio)
      : '';

    calendario.fechaFinTxt = calendario.fechaFin
      ? this.formatFecha(calendario.fechaFin)
      : '';

    crearCalendario(calendario)
      .then(res => {
        //@ts-ignore
        calendario.id = res.data;
        ...
        this.lstCalendario.push(calendario);
        this.gridApiCalendario.setRowData(this.lstCalendario);
