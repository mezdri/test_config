...
const makeHttpRequest = (): AxiosPromise<TallerBackend[]> => {
  ...
  return axios.http.get(`mantenimientopreventivo/talleres/${clienteId}`);
};
...
// .then(
//   (Pautas): Promise<Array<PautaActividadBackend | null>> =>
//     Promise.all(Pautas.map(getMaybeValidPauta))
// )
// .then(Pautas => Pautas.filter(x => x))
// .then(Pautas => Pautas.map(parsePauta))
